...
public:
    ...
    BOOL Show(int nCmdShow) {
        return ShowWindow(hWindow, nCmdShow);
    }
    BOOL Update() {
        return UpdateWindow(hWindow);
    }
    BOOL Redraw(bool erase = true) {
        return RedrawWindow(hWindow, NULL, NULL, RDW_INVALIDATE | (erase ? RDW_ERASE : 0));
    }

protected:
    HWND hWindow;

    Window(HWND hWindow = NULL)
        : hWindow(hWindow){};
